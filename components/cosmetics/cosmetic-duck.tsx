...
/* eslint-disable @next/next/no-img-element -- modular SVG layers share one canonical 512x512 coordinate system */
...
export function CosmeticDuck({
  ...
  return (
    <div
      ...
      className={`relative shrink-0 select-none overflow-visible ${animate ? 'duck-avatar-rig-idle' : ''}`}
    ...
    >
      ...
      {layers.map((item) => (
        <img
          ...
          src={item.asset}
          alt=""
          className={`pointer-events-none absolute inset-0 h-full w-full object-contain cosmetic-layer-${item.slot}`}
        />
      ))}
    </div>
  )
