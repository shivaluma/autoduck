    ...
    <div className="grid md:grid-cols-[280px_1fr]">
      ...
      <div className="p-5">
        ...
        <div className="mt-4 grid grid-cols-3 gap-2 sm:grid-cols-4 lg:grid-cols-6">
          ...
          {choices.map((item) => <button key={item.id} title={item.name} onClick={() => equip(item.id)} className={`relative aspect-square overflow-hidden rounded-xl border-2 bg-black/20 ${appearance[`${slot}Id` as keyof DuckAppearance] === item.id ? 'border-[var(--color-ggd-gold)]' : 'border-white/10'}`}><img src={item.asset} alt={item.name} className="h-full w-full object-contain" /><span className="absolute inset-x-1 bottom-1 truncate rounded bg-black/65 px-1 text-[9px] font-black">{item.name}</span></button>)}
