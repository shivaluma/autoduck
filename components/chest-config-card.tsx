...
interface ChestConfigOption {
  userId: number
  name: string
  shieldCount?: number
}

interface ChestConfigCardProps {
  chestId: number
  ownerName: string
  effect: ChestEffect
  description: string
  targetUserId?: number
  targetOptions: ChestConfigOption[]
  needsTarget: boolean
  onTargetChange: (targetUserId?: number) => void
}

export function ChestConfigCard({
  chestId,
  ownerName,
  effect,
  description,
    ...
    <div className="rounded-2xl border-3 border-[var(--color-ggd-outline)] bg-[var(--color-ggd-panel)] p-4 shadow-[0_4px_0_var(--color-ggd-outline)]">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="font-display text-lg text-white truncate">🎁 Rương #{chestId} · {ownerName}</div>
          <p className="font-body text-sm text-white/75 mt-2">{description}</p>
        </div>
        <ChestCard effect={effect} chestId={chestId} size="sm" opened animated={false} />
      </div>
