...
export function Season3RaceView({ raceId }: { raceId: number }) {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      void fetch(`/api/races/${raceId}`).then(async (response) => {
        if (!active || !response.ok) return
        const nextRace = await response.json() as RaceStatus
        if (nextRace.status === 'finished' || nextRace.status === 'failed') return
        setRace(nextRace)
      })
    }, 2500)
    return () => {
  ...
  return <main className="mx-auto min-h-screen max-w-5xl space-y-6 p-6 text-white">
    <header className="flex flex-wrap items-center justify-between gap-3"><div><div className="text-xs font-black tracking-[0.25em] text-[var(--color-ggd-gold)]">ĐUA DZỊT · SEASON 3</div><h1 className="mt-1 font-display text-4xl">🏁 Duck Duck Race #{raceId}</h1></div><Link href="/season-3" className="rounded-xl border-2 border-white/20 px-4 py-2 font-black">POND</Link></header>
    {isLive && <><section className="rounded-3xl border-4 border-[var(--color-ggd-outline)] bg-[var(--color-ggd-panel)] p-6 text-center"><div className="font-display text-4xl text-[var(--color-ggd-neon-green)]">RACE ĐANG CHẠY 🏃💨</div><p className="mt-2 text-white/65">BXH tự cập nhật khi race kết thúc.</p></section>{race.engine ? <PhaserRaceCanvas raceId={raceId} players={racePlayers} chaosType={race.engine.chaosConfig?.type} liveConfig={race.engine.config} liveSyncTick={race.engine.liveTick ?? undefined} liveManualInputs={race.engine.liveManualInputs} /> : <RaceLiveView raceId={raceId} season3Mode />}</>}
    ...
    {race.status === 'finished' && <><RaceCelebration duration={5000} allPlayers={officialRanking.map((player) => ({ name: player.displayName ?? player.name, avatarUrl: player.avatarUrl, gotScar: player.gotScar, usedShield: player.usedShield, initialRank: player.initialRank }))} victims={officialRanking.filter((player) => player.gotScar).map((player) => ({ name: player.displayName ?? player.name, avatarUrl: player.avatarUrl }))} verdict={race.finalVerdict} /><section className="rounded-3xl border-4 border-[var(--color-ggd-outline)] bg-[var(--color-ggd-panel)] p-6 text-center"><div className="text-5xl">📰</div><div className="mt-2 font-display text-3xl">RACE COMPLETE</div><p className="mt-3 text-lg font-black text-[var(--color-ggd-gold)]">{race.finalVerdict}</p><p className="mt-2 text-white/60">Chaos, Shield, King và Prediction đã được resolve.</p></section>{race.engine?.config && <Season3ReplayPlayer key={raceId} raceId={raceId} players={racePlayers} config={race.engine.config} events={race.engine.events} resultDigest={race.engine.resultDigest} />}{race.engine?.config && race.engine.events.length > 0 && <PostRaceStatsPanel config={race.engine.config} events={race.engine.events} players={racePlayers} />}{race.videoUrl && <section className="overflow-hidden rounded-3xl border-4 border-[var(--color-ggd-outline)] bg-black"><div className="border-b-2 border-white/10 px-5 py-4 font-black text-white">🎬 RACE REPLAY</div><video src={race.videoUrl} controls playsInline className="w-full" /></section>}<section className="overflow-hidden rounded-3xl border-4 border-[var(--color-ggd-outline)] bg-[var(--color-ggd-panel)]"><div className="border-b-2 border-white/10 px-5 py-4 font-black">FINAL RANKING</div>{officialRanking.map((player, index) => <div key={`${player.userId}-${player.cloneIndex ?? 'main'}`} className="flex items-center gap-3 border-b border-white/10 px-5 py-4 last:border-0"><span className="w-8 text-2xl font-black text-white/45">{index + 1}</span><Season3Avatar name={player.displayName ?? player.name} avatarUrl={player.avatarUrl} size={40} /><span className="flex-1 font-black">{player.displayName ?? player.name}</span>{player.usedShield && <span className="rounded-full bg-[var(--color-ggd-sky)] px-2 py-1 text-xs font-black text-[var(--color-ggd-outline)]">🛡️ ĐÃ DÙNG</span>}{player.gotScar && <span className="rounded-full bg-[var(--color-ggd-orange)] px-2 py-1 text-xs font-black">BỊ LÀM DZỊT</span>}</div>)}</section>{ghostDucks.length > 0 && <section className="overflow-hidden rounded-3xl border-4 border-dashed border-white/20 bg-black/20"><div className="border-b border-white/10 px-5 py-4 font-black text-white/70">👻 GHOST DUCK — GÂY NHIỄU (KHÔNG TÍNH BXH)</div>{ghostDucks.map((player) => <div key={`ghost-${player.userId}`} className="flex items-center gap-3 border-b border-white/10 px-5 py-4 last:border-0"><span className="w-8 text-xl font-black text-white/30">👻</span><Season3Avatar name={player.displayName ?? player.name} avatarUrl={player.avatarUrl} size={40} /><span className="flex-1 font-black text-white/75">{player.displayName ?? player.name}</span><span className="rounded-full bg-white/10 px-2 py-1 text-xs font-black text-white/55">KHÔNG BỊ LÀM DZỊT</span>{player.initialRank && <span className="text-xs text-white/40">track #{player.initialRank}</span>}</div>)}</section>}{race.commentaries.length > 0 && <section className="rounded-3xl border-4 border-[var(--color-ggd-outline)] bg-[var(--color-ggd-panel)] p-5"><h2 className="font-display text-2xl">🎤 MC Vịt</h2><div className="mt-4 space-y-2">{race.commentaries.map((commentary) => <p key={`${commentary.timestamp}-${commentary.content}`} className="rounded-xl bg-black/20 p-3 text-white/80">{commentary.content}</p>)}</div></section>}</>}
  </main>
}
