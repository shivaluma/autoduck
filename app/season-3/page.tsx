...
type SeasonData = {
  ...
  currentWeek: { id: number; weekNumber: number; status: string; chaosType: string; chaosTargetName: string | null; chaosGroups?: number[][]; predictionCount: number; predictionSubmitted: boolean } | null
  ...
  return <main className="mx-auto max-w-6xl space-y-6 p-4 pb-12 text-white sm:p-6 lg:p-8">
    <header className="relative overflow-hidden rounded-[2rem] border-4 border-[var(--color-ggd-outline)] bg-[radial-gradient(circle_at_85%_15%,rgba(61,255,143,.22),transparent_34%),linear-gradient(135deg,#241548,#110b24)] p-6 shadow-[0_8px_0_var(--color-ggd-outline)] sm:p-8">
      ...
      <div className="relative z-[1] flex flex-wrap items-start justify-between gap-5">
        <div><div className="inline-flex items-center gap-2 rounded-full border border-[var(--color-ggd-neon-green)]/40 bg-[var(--color-ggd-neon-green)]/10 px-3 py-1 text-xs font-black tracking-[0.2em] text-[var(--color-ggd-neon-green)]"><span className="h-2 w-2 animate-pulse rounded-full bg-[var(--color-ggd-neon-green)]" /> SEASON {data.season.year}</div><h1 className="mt-3 font-display text-5xl leading-none sm:text-6xl">ĐUA DZỊT <span className="text-[var(--color-ggd-gold)]">S3</span></h1><p className="mt-3 max-w-2xl text-sm font-bold leading-relaxed text-white/65">Race chạy vanilla 100%. Sau khi host nhập BXH, pond mới resolve Chaos → Sẹo → Shield → King → Prediction.</p></div>
        <div className="rounded-2xl border-2 border-white/15 bg-black/25 p-4 text-right"><div className="text-[10px] font-black tracking-widest text-white/45">SEASON FORMAT</div><div className="mt-1 font-display text-3xl">{data.season.weeks} WEEKS</div><div className="mt-1 text-xs text-white/55">1 chaos / week</div></div>
      </div>
      <div className="relative z-[1] mt-7 grid grid-cols-4 gap-2 border-t border-white/10 pt-5 text-center"><div><div className="font-display text-2xl text-[var(--color-ggd-gold)]">🎴</div><div className="text-[10px] font-black text-white/50">CHAOS</div></div><div><div className="font-display text-2xl text-[var(--color-ggd-orange)]">🩹</div><div className="text-[10px] font-black text-white/50">SCARS</div></div><div><div className="font-display text-2xl text-[var(--color-ggd-neon-green)]">👑</div><div className="text-[10px] font-black text-white/50">KING</div></div><div><div className="font-display text-2xl text-[var(--color-ggd-lavender)]">🔮</div><div className="text-[10px] font-black text-white/50">PREDICT</div></div></div>
    </header>
