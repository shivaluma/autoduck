...
export default function Season3Page() {
  ...
  return <main className="mx-auto max-w-6xl space-y-6 p-4 pb-12 text-white sm:p-6 lg:p-8">
    ...
    {data.viewer && week?.status === 'open' && !week.viewerSkipped && <section className="rounded-[2rem] border-4 border-[var(--color-ggd-neon-green)]/70 bg-[var(--color-ggd-panel)] p-5 shadow-[0_6px_0_var(--color-ggd-outline)]"><div className="flex flex-wrap items-end justify-between gap-3"><div><div className="text-xs font-black tracking-[0.2em] text-[var(--color-ggd-neon-green)]">RACE PREP</div><h2 className="font-display text-3xl">🎒 Chọn loadout</h2></div><div className="font-black text-[var(--color-ggd-gold)]">{selectedCost}/3 Prep Credits</div></div><p className="mt-3 text-sm text-white/60">⚡ Speed · 🛡️ Defense · 💥 Attack — soft counter triangle. Không bonus stat mono-build; badge chỉ cosmetic.</p><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{data.raceItems.map((item) => { const selected = selectedItems.includes(item.id); const disabled = !selected && (selectedItems.length >= 2 || selectedCost + item.cost > 3 || (item.category === 'major' && selectedMajor)); return <button key={item.id} disabled={disabled} onClick={() => toggleItem(item.id)} className={`rounded-2xl border-2 p-4 text-left transition disabled:cursor-not-allowed disabled:opacity-30 ${selected ? 'border-[var(--color-ggd-neon-green)] bg-[var(--color-ggd-neon-green)]/15' : 'border-white/10 bg-black/20 hover:border-white/35'}`}><div className="flex items-center gap-3"><span className="text-3xl">{item.icon}</span><div><div className="font-black">{item.name}</div><div className="text-xs font-bold text-[var(--color-ggd-gold)]">{item.cost} Credit · {item.category}</div></div>{selected && <span className="ml-auto">✓</span>}</div><p className="mt-2 text-xs text-white/55">{item.description}</p></button>})}</div>{loadoutHint && <div className={`mt-4 rounded-xl border-2 px-4 py-3 text-sm font-bold ${loadoutHint.tier === 'recommended' ? 'border-[var(--color-ggd-neon-green)] bg-[var(--color-ggd-neon-green)]/10 text-[var(--color-ggd-neon-green)]' : 'border-[var(--color-ggd-lavender)] bg-[var(--color-ggd-lavender)]/10 text-[var(--color-ggd-lavender)]'}`}>{loadoutHint.badge && <div className="text-base">{loadoutHint.badge}{loadoutHint.label ? ` · ${loadoutHint.label}` : ''}</div>}<div className="mt-1 font-normal text-white/80">{loadoutHint.message}</div></div>}<button disabled={selectedCost !== 3 || selectedItems.length !== 2} onClick={() => void saveLoadout()} className="mt-4 w-full rounded-xl bg-[var(--color-ggd-neon-green)] px-5 py-3 font-black text-[var(--color-ggd-outline)] disabled:cursor-not-allowed disabled:opacity-30">{week.loadout.status === 'ready' ? '✓ UPDATE LOADOUT' : '🔒 LOCK LOADOUT'}</button></section>}
