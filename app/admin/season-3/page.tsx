...
export default function AdminSeason3Page() {
  ...
  return <main className="mx-auto max-w-6xl space-y-6 p-6 text-white">
    ...
    {!data?.season ? <section className="rounded-3xl border-4 border-[var(--color-ggd-outline)] bg-[var(--color-ggd-surface-2)] p-5">
      ...
      {(data.balance?.pickups.length ?? 0) > 0 && <section className="rounded-3xl border-4 border-[var(--color-ggd-outline)] bg-[var(--color-ggd-surface-2)] p-5"><div className="text-xs font-black tracking-[0.2em] text-[var(--color-ggd-neon-green)]">WILD ITEM TELEMETRY</div><h2 className="font-display text-2xl">📦 Pickup Balance</h2><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[700px] text-left text-sm"><thead className="text-white/45"><tr><th className="pb-2">ITEM</th><th>PICKS</th><th>ACTIVATE</th><th>HIT</th><th>MANUAL</th><th>AUTO</th><th>Δ RANK</th></tr></thead><tbody>{data.balance?.pickups.map((item) => <tr key={item.name} className="border-t border-white/10"><td className="py-3 font-black">{item.name.replaceAll('_', ' ')}</td><td>{item.picks}</td><td>{(item.activationRate * 100).toFixed(1)}%</td><td>{(item.hitRate * 100).toFixed(1)}%</td><td>{(item.manualRate * 100).toFixed(1)}%</td><td>{(item.autoRate * 100).toFixed(1)}%</td><td className={Math.abs(item.averageRankDelta) > 1 ? 'text-[var(--color-ggd-orange)]' : ''}>{item.averageRankDelta > 0 ? '+' : ''}{item.averageRankDelta.toFixed(2)}</td></tr>)}</tbody></table></div></section>}
    </>}
  </main>
}
