  ...
  return <main className="mx-auto max-w-6xl space-y-6 p-6 text-white">
    ...
    {!data?.season ? <section className="rounded-3xl border-4 border-[var(--color-ggd-outline)] bg-[var(--color-ggd-surface-2)] p-5">
      ...
      {currentWeek && <section className="rounded-3xl border-4 border-[var(--color-ggd-outline)] bg-[var(--color-ggd-panel)] p-5">
        ...
        {currentWeek.status === 'locked' && <div className="mt-5 flex flex-wrap items-center gap-3"><button onClick={() => void act({ action: 'start-race', weekId: currentWeek.id })} className="rounded-xl bg-[var(--color-ggd-neon-green)] px-5 py-3 font-black text-[var(--color-ggd-outline)]">🏁 START DUCK RACE</button><button onClick={() => void act({ action: 'start-race', weekId: currentWeek.id, test: true, secret })} className="rounded-xl border-2 border-[var(--color-ggd-gold)] px-5 py-3 font-black text-[var(--color-ggd-gold)]">🧪 TEST RACE</button><span className="text-sm text-white/60">🗓️ Race thật: Thứ Hai (giờ Việt Nam) · Test: không đổi stats/point.</span></div>}
